...
public static class SchedulesDeleteController
{
    public static void AddEndPointSchedulesDelete(this WebApplication app)
    {
        ...
        scheduleGroup.MapDelete("/day/between",
            [Authorize]([FromServices] Dal<Schedule> scheduleDal,
                [FromBody] ScheduleDayBetweenRequest scheduleRequest) =>
            {
                try
                {
                    var schedulesDay = scheduleDal.SearchForDay(a =>
                        a.Date >= scheduleRequest.startDay && a.Date <= scheduleRequest.endDay);
            ...
            }).WithSwaggerDocumentation("Deleting schedules between days",
            "The schedules between one day to another will be deleted");
